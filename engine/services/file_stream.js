...
app = angular.module('app.service.file_stream', []);

app.factory('fileStream',function () {
    ...
    var config = {
        ...
        tokenURL : __UPLOAD_TOKEN_URL__, /** 根据文件名、大小等信息获取Token的URI（用于生成断点续传、跨域的令牌） */
        frmUploadURL : __UPLOAD_FLASH_URL__, /** Flash上传的URI */
        uploadURL : __UPLOAD_URL__ /** HTML5上传的URI */
        //simLimit: 200, /** 单次最大上传文件个数 */
        //extFilters: [".txt", ".rpm", ".rmvb", ".gz", ".rar", ".zip", ".avi", ".mkv", ".mp3"], /** 允许的文件扩展名, 默认: [] */
        //onSelect: function(list) {alert('onSelect'); log(list);}, /** 选择文件后的响应事件 */
        ...
        //onComplete: function(file) {alert('onComplete');log(file)}, /** 单个文件上传完毕的响应事件 */
